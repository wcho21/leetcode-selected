...
The base case is when there is no letters left to make combination, and we return a list containing the empty string in this case.

Now assume that we're given a string $a_1 a_2 \dots a_N$ of length $N$, and we've solved the problem for $a_{i+1} \dots a_N$.
Since we have the combinations for the substring $a_{i+1} \dots a_N$, simply prepend each letter to all the combinations and return the results.
...
For each character, we iterate over the constructed combinations, and for each combination, iterate over each mapped letter.
...
(We can easily find that the term $M^N$ dominates in the geometric sum $M \frac{M^N-1}{M-1}$ since $M>1$.)
